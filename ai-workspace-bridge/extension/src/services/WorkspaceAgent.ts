...
const AGENT_TOOLS: AgentTool[] = [
  ...
  {
    name: "runTerminal",
    trigger: "run <command> / execute <command>",
    access: "Runs a shell command inside the workspace using Node child_process.",
    safety: "Requires confirmation, blocks destructive commands, and caps output/time."
  },
  {
    name: "chatgptBrowser",
...
export class WorkspaceAgent {
  ...
  public getToolManifestForModel(): string {
    ...
    return [
      ...
      "- runTerminal: {\"command\":\"shell command\",\"stdin\":\"optional piped input\"}",
      "",
    ...
    switch (call.tool) {
      ...
      case "runTerminal":
        return this.runTerminal(
          this.getStringArg(args, "command"),
          this.getOptionalStringArg(args, "stdin"),
          emit
        );
      default:
        return "ERROR: Unknown tool \"" + call.tool + "\". Available tools: listFiles, readFile, writeFile, searchWorkspace, summarizeWorkspace, runTerminal. Use TOOL_CALL with one of these tool names only.";
    }
  }
  ...
  private async runTerminal(command: string, stdin?: string, emit?: (chunk: string) => void): Promise<string> {
    const folder = this.getWorkspaceFolder();
    ...
    if (this.isBlockedCommand(trimmed)) {
      return "Blocked command. This tool refuses destructive or system-level operations.";
    }

    const interactiveHint = await this.detectInteractivePythonScript(trimmed);

    if (interactiveHint && !stdin) {
      const message = [
        "Interactive script detected before execution.",
        interactiveHint,
        "Re-run with tool arg stdin (for example: \"5\\n\") or change the script to avoid input()."
      ].join(" ");
    ...
    if (answer !== "Run") {
      return "Cancelled command execution.";
    }

    try {
      emit?.(`\n**Terminal started**: \`${trimmed}\`\n\n`);
      emit?.(`PS ${folder.uri.fsPath}> ${trimmed}\n`);

      const result = await this.terminalAgent.run(
        trimmed,
        folder.uri.fsPath,
        TERMINAL_TIMEOUT_MS,
      ...
      this.logger.error(`Terminal command failed: ${trimmed}`, error);
  ...
  private getOptionalStringArg(args: Record<string, unknown>, key: string): string | undefined {
    const value = args[key];
  ...
  private async detectInteractivePythonScript(command: string): Promise<string | null> {
    const match = command.match(/\bpython(?:3|\.exe)?\b\s+(?:"([^"]+\.py)"|'([^']+\.py)'|([^\s]+\.py))/i);
    ...
    try {
      const uri = this.resolveWorkspaceFile(scriptPath);
      ...
      if (/\binput\s*\(/i.test(text) || /\braw_input\s*\(/i.test(text)) {
        return `Script \`${scriptPath}\` uses input().`;
      }
